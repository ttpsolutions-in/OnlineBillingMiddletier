...
ETradersApp.controller("MaterialsController", ['GlobalVariableService', '$scope', '$filter', '$http', '$location', '$routeParams', 'toaster', 'CommonService', 'uiGridConstants',
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            { width:250, displayName: 'Material Name', field: 'DisplayName', cellTooltip: true, enableCellEdit: false, cellClass: 'text-left', headerCellClass: 'text-center' },
            ...
            { width: 100, displayName: 'Active', field: 'Active', enableCellEdit: false, cellTooltip: true, cellClass: 'text-right', headerCellClass: 'text-center' },
            {
    ...
    $scope.SaveMaterials = function (isFormValid) {
        try {
            ...
            if (isValid) {
                var displayName = '';
                var size = $scope.Material.Size1 + $scope.Material.Size2;
                if (size.trim().length == 0 && $scope.Material.Model.length == 0) {
                    toaster.pop('error', "", "Either Model or Size1 or Size2 is required", 5000, 'trustedHtml');
                    return;
                }
                if (size.trim().length == 0)
                    displayName = strItemCategoryName.substring(0, strItemCategoryName.indexOf(' ')) + '-' + strItemDescription.substring(0, strItemDescription.indexOf(' ')) + ' ' + $scope.Material.Model
                else
                ...
                var values = {
                    ...
                    , "CreatedOn":new Date()
                    , "Product": $scope.Material.Product.toString()
