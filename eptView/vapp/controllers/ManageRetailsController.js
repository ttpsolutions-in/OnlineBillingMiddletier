ETradersApp.controller("ManageRetailsController", ['GlobalVariableService', 'PrintService', '$scope', '$filter', '$q', '$http', '$location', '$routeParams',
    '$timeout', 'toaster', 'CommonService', 'uiGridConstants', 'uiGridValidateService', function (GlobalVariableService, PrintService, $scope, $filter, $q, $http, $location, $routeParams, $timeout, toaster, CommonService, uiGridConstants, uiGridValidateService) {
        ...
        $scope.GetGrandAmount = function () {
            ...
            return $scope.RetailData.TotalAmount.toFixed(2);
        };
        ...
        $scope.CalculateGST = function () {
            ...
            /*$scope.RetailData.GrandAmount = $scope.RetailData.TotalAmount;
            var isChecked = $scope.RetailData.GSTApplied;
            if (isChecked) {
                $scope.RetailData.GSTAmount = $scope.RetailData.TotalAmount * $scope.RetailData.GSTPercentage / 100;
                $scope.RetailData.GrandAmount = ($scope.RetailData.TotalAmount + $scope.RetailData.GSTAmount).toFixed(2);
            } else {
                $scope.RetailData.GrandAmount = ($scope.RetailData.GrandAmount - $scope.RetailData.GSTAmount).toFixed(2);
                $scope.RetailData.GSTAmount = 0;
            }
            return $scope.RetailData.GSTAmount;
            */
        };
        ...
        $scope.SubmitItems = function (isFormValid, callback) {
            try {
                ...
                if (isValid) {
                    ...
                    toaster.pop('warning', "", "Please select Items for submit", 5000, 'trustedHtml');
                }
            } catch (error) {
